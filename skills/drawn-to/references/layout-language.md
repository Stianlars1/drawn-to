...
10. **Micro / chip cell** (Tier C observation, source scale unknown). The glyph occupies about 38% of its plate width; remaining space is roughly 31% per side. A proposed 48px CSS tile would therefore have an 18px glyph and about 15px side space, but 48px is an implementation choice, not a recovered source value. At chip scale, keep one readable index mark and remove scene layers. Subtle tone and a small edge highlight can separate the tile; inspect it at its actual size. Repeated chips organize a list and do not need different explanatory devices.
11. **Portrait / row-span cell** (Tier C). The capture's width:height is roughly 372:538, not an established CSS size. Graphic ink spans nearly the full width and about 59% of height. This source uses a three-station process to occupy the vertical space. A tall cell can also serve a different appropriate subject; it is not required to depict a sequence.
...
## Recipe defaults for the matching layout family

Apply these to the referenced anatomy, not every page. The approved brief and
